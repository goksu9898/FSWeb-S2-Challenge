...
function cumlelereDonustur(cumleler, ayrac) {
  ayrac = ayrac || ",";
  var birlesikCumleler = [];
  for (var i = 0; i < cumleler.length; i++) {
    var x = cumleler[i].join(ayrac);
...
function paragrafOlustur(dizi, cbCumleKur, cbCumlelereDonustur) {
  ...
  const tekCumlelerArray = cumleArray.filter((eleman, index) => {
...
function emojileriDonustur(msj, omojilerKlavuzu) {
for (let shrt in omojilerKlavuzu) {
  msj = msj.replaceAll(shrt, omojilerKlavuzu[shrt]);
  msj = msj.replaceAll(shrt.toUpperCase(), omojilerKlavuzu[shrt]);
}
